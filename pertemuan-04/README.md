# Pertemuan 4 — Metadata dan Interoperabilitas

## Identitas sumber

- **Judul:** Penerapan SPARQL dan Ontology pada Pencarian Data Buku dan Perbandingannya dengan Pendekatan Relasional
- **Pembuat:** Nibras Syihabil Haq dan Kemas Muslim Lhaksmana
- **URI sumber:** https://openlibrarypublications.telkomuniversity.ac.id/index.php/engineering/id/article/view/6718
- **Jenis sumber:** Artikel ilmiah
- **Deskripsi:** Artikel ini membahas penerapan ontologi, RDF, dan SPARQL dalam pencarian data buku serta membandingkannya dengan pendekatan relasional.
- **Tanggal:** 2018-08-01
- **Bahasa:** id
- **Hak:** Hak cipta milik penulis dan penerbit eProceedings of Engineering

## Pemetaan Dublin Core Terms

| Dublin Core Terms | Nilai |
|---|---|
| dcterms:title | Penerapan SPARQL dan Ontology pada Pencarian Data Buku dan Perbandingannya dengan Pendekatan Relasional |
| dcterms:creator | Nibras Syihabil Haq; Kemas Muslim Lhaksmana |
| dcterms:identifier | https://openlibrarypublications.telkomuniversity.ac.id/index.php/engineering/id/article/view/6718 |
| dcterms:type | Artikel ilmiah |
| dcterms:description | Artikel ini membahas penerapan ontologi, RDF, dan SPARQL dalam pencarian data buku serta membandingkannya dengan pendekatan relasional. |
| dcterms:date | 2018-08-01 |
| dcterms:language | id |
| dcterms:subject | SPARQL; Ontology; RDF; Pencarian Data Buku |
